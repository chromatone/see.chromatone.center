...
__export(use_exports, {
  ...
  devices: () => devices,
  download: () => download,
  ensureDevicesListPermissions: () => ensureDevicesListPermissions,
  fileNames: () => fileNames,
  forwardMidi: () => forwardMidi,
  freqColor: () => freqColor,
  freqPitch: () => freqPitch,
  getCents: () => getCents,
  ...
  midi: () => midi,
  midiA: () => midiA,
  midiAttack: () => midiAttack,
  midiOnce: () => midiOnce2,
  midiPlay: () => midiPlay,
  midiRelease: () => midiRelease,
  midiStop: () => midiStop,
  mimeExtMap: () => mimeExtMap,
  mimeType: () => mimeType,
  noteNames: () => noteNames,
  notes: () => notes,
  ...
  pianoReleaseAll: () => pianoReleaseAll,
  pitchColor: () => pitchColor,
  pitchFreq: () => pitchFreq,
  pitchNoteOctave: () => pitchNoteOctave,
  playChroma: () => playChroma,
  playChromaOnce: () => playChromaOnce,
  playKey: () => playKey,
  playNote: () => playNote,
  quantizeModes: () => quantizeModes,
  record: () => record,
  ...
  stopAll: () => stopAll,
  stopChroma: () => stopChroma,
  stopNote: () => stopNote,
  synth: () => synth,
  synthAttack: () => synthAttack,
  synthOnce: () => synthOnce,
  synthRelease: () => synthRelease,
  synthReleaseAll: () => synthReleaseAll,
  tap: () => tap,
  tempo: () => tempo,
  tracks: () => tracks,
  tuner: () => tuner,
...
var Aubio = function(Aubio2) {
  ...
  aa = typeof process === "object" && typeof require === "function" && !u && !v;
  ba = !u && !aa && !v;
  var w = "";
  function ca(a) {
...
var currentCamera = (0, import_core2.useStorage)("cast-camera", "default");
var currentMic = (0, import_core2.useStorage)("cast-mic", "default");
var showRecordingDialog = ref();
var recordingName = ref("");
var recordCamera = ref(true);
var mimeType = (0, import_core2.useStorage)("slidev-record-mimetype", "video/webm");
var mimeExtMap = {
  "video/webm": "webm",
...
function useRecording() {
  ...
  const timestamp2 = (0, import_core.useTimestamp)();
  ...
  (0, import_core2.useEventListener)("beforeunload", (event) => {
    if (!recording2.value)
      return;
...
var toggled = (0, import_core3.useLastChanged)(recording);
var timestamp = (0, import_core3.useTimestamp)();
...
function useRecorder() {
  if (!recorder) {
    recorder = new import_tone.Recorder();
  }
...
function useAudio() {
  if (!audio.initiated) {
    (0, import_tone2.start)();
    const { recorder: recorder2 } = useRecorder();
    master.stream = import_tone2.context.createMediaStreamDestination();
    master.meter = new import_tone2.Meter().toDestination();
    ...
    master.limiter = new import_tone2.Limiter(-18).connect(master.meter);
    master.reverb = new import_tone2.Reverb({
      decay: 1,
      wet: 0.5
    }).connect(master.meter);
    master.limiter.connect(master.reverb);
    watchEffect(() => {
      (0, import_tone2.getDestination)().mute = audio.mute;
    });
    watchEffect(() => {
      (0, import_tone2.getDestination)().volume.targetRampTo((0, import_tone2.gainToDb)(audio.volume), 0.1);
    });
...
function createChannel(title = (Math.random() * 1e3).toFixed(0), options2) {
  ...
  const channel = new import_tone2.Limiter(options2).connect(master2.limiter);
...
var import_webmidi = require("webmidi");
...
// .vitepress/use/midi.js
var import_core5 = require("@vueuse/core");
var midi = reactive({
  enabled: false,
  initiated: false,
  out: true,
  ...
  playing: false,
  channels: {},
  channel: (0, import_core4.useStorage)("global-midi-channel", 1),
  note: {
    pitch: 0,
    channel: 1
  },
  offset: (0, import_core5.useClamp)(0, -2, 2),
  keyboard: true,
  cc: {},
  ccLearn: {},
  message: null,
  log: [],
  clock: 0,
  filter: (0, import_core4.useStorage)("global-midi-filter", {}),
...
function playKey(name, offset = 0, off, velocity = 1) {
  ...
  const note = new import_webmidi.Note(noteName, {
...
function useMidi() {
  if (!midi.initiated) {
    setupKeyboard();
    onMounted(() => {
      if (import_webmidi.WebMidi.supported) {
        setupMidi();
      }
    });
    watchEffect(() => {
      if (!midi.out)
        return;
      let outs = Object.values(import_webmidi.WebMidi.outputs);
      if (midi.playing) {
        outs.forEach((output) => {
          output.sendContinue();
        });
  ...
  return {
    ...
    midiOnce: midiOnce2,
    setCC,
    WebMidi: import_webmidi.WebMidi
  };
}
function setupMidi() {
  import_webmidi.WebMidi.enable();
  import_webmidi.WebMidi.addListener("enabled", (e) => {
    midi.enabled = true;
    initMidi();
  });
  import_webmidi.WebMidi.addListener("connected", (e) => {
    initMidi();
  });
  import_webmidi.WebMidi.addListener("disconnected", (e) => {
...
function initMidi() {
  ...
  import_webmidi.WebMidi.inputs.forEach((input2) => {
    midi.enabled = true;
  ...
  import_webmidi.WebMidi.outputs.forEach((output) => {
...
function forwardMidi(iid, oid) {
  var _a, _b;
  const output = import_webmidi.WebMidi.outputs.find((out) => out.id == oid);
...
var import_core6 = require("@vueuse/core");
var import_core7 = require("@vueuse/core");
var import_core8 = require("@vueuse/core");
var quantizeModes = ["+0", "@8n", "@16n", "@32n"];
var synth = {
  state: reactive({
    midi: (0, import_core6.useStorage)("synth-midi", true),
    initiated: false,
    mute: false,
    quantize: (0, import_core6.useCycleList)(quantizeModes, { initialValue: "+0" }),
    volume: (0, import_core8.useClamp)(1, 0, 2)
  }),
  params: reactive({
    maxPolyphony: 50,
    oscillator: {
      type: (0, import_core6.useStorage)("synth-osc", "sawtooth8")
    },
    volume: -30,
    envelope: {
      attack: 0.01,
...
function useSynth() {
  if (!synth.state.initiated) {
    (0, import_core7.onKeyDown)("Escape", () => {
      synthReleaseAll();
    });
    watch(() => synth.state.volume, (vol) => synth.poly && synth.poly.volume.rampTo((0, import_tone3.gainToDb)(vol)));
    ...
    watch(() => midi.note, (note) => {
      if (!synth.state.midi)
        return;
      if (note.velocity > 0) {
        synthAttack((0, import_tone3.Midi)(note.number).toFrequency(), note.velocity / 127);
      } else {
        synthRelease((0, import_tone3.Midi)(note.number).toFrequency());
      }
    });
...
function init() {
  (0, import_tone3.start)();
  if (synth == null ? void 0 : synth.poly)
    return;
  const { channel } = createChannel("synth");
  synth.widener = new import_tone3.StereoWidener(0.7).connect(channel);
  synth.reverb = new import_tone3.Reverb(3).connect(synth.widener);
  synth.delay = new import_tone3.PingPongDelay({ delayTime: "16n", feedback: 0.3, wet: 0.3, maxDelay: "4n" }).connect(synth.widener);
  synth.pan = new import_tone3.AutoPanner({ frequency: "4n", depth: 0.4 }).connect(synth.reverb).connect(synth.delay).connect(synth.widener);
  synth.compressor = new import_tone3.Compressor().connect(synth.pan);
  synth.poly = new import_tone3.PolySynth(import_tone3.MonoSynth, synth.params).connect(synth.compressor);
  synth.pan.start();
}
...
var import_tonal = require("@tonaljs/tonal");
import_tonal.ChordType.add(["1P", "2m"], ["2m"], "minor second");
import_tonal.ChordType.add(["1P", "2M"], ["2M"], "major second");
import_tonal.ChordType.add(["1P", "3m"], ["3m"], "minor third");
import_tonal.ChordType.add(["1P", "3M"], ["3M"], "major third");
import_tonal.ChordType.add(["1P", "4P"], ["4P"], "perfect fourth");
import_tonal.ChordType.add(["1P", "5d"], ["TT"], "tritone");
import_tonal.ChordType.add(["1P", "5P"], ["5P"], "perfect fifth");
import_tonal.ChordType.add(["1P", "6m"], ["6m"], "minor sixth");
import_tonal.ChordType.add(["1P", "6M"], ["6M"], "major sixth");
import_tonal.ChordType.add(["1P", "7m"], ["7m"], "minor seventh");
import_tonal.ChordType.add(["1P", "7M"], ["7M"], "major seventh");
var chordType = import_tonal.ChordType;
var scaleType = import_tonal.ScaleType;
var chordList = import_tonal.ChordType.all();
var scaleList = import_tonal.ScaleType.all();
...
// .vitepress/use/chroma.js
var import_tonal2 = require("@tonaljs/tonal");
var import_core9 = require("@vueuse/core");
var allNotes = [...notes].map((n, i) => ({ name: n, pitch: i }));
var globalScale = reactive({
  tonic: (0, import_core9.useClamp)(useStorage("global-tonic", 0), 0, 11),
  note: computed(() => allNotes[globalScale.tonic]),
  chroma: useStorage("global-chroma", "101011010101"),
  set: computed(() => import_tonal2.ScaleType.get(globalScale.chroma)),
  full: computed(() => {
    let sc = globalScale.note.name + "4 " + globalScale.set.name;
    return import_tonal2.Scale.get(sc);
  }),
  pcs: computed(() => import_tonal2.Scale.scaleNotes(globalScale.full.notes)),
  isIn: computed(() => import_tonal2.Pcset.isNoteIncludedIn(globalScale.pcs))
});
function getChromaNotes(chroma = "100010010000", tonic = globalScale.tonic) {
  ...
  let chOct = rotateArray(allNotes, -tonic).map((n, i) => {
    let noteName = (0, import_tone4.Frequency)(n.pitch + tonic + 57, "midi").toNote();
    return noteName;
  });
  ...
  return import_tonal2.Note.sortedNames(filtered);
}
...
var import_lch = __toESM(require("colord/plugins/lch"));
var import_mix = __toESM(require("colord/plugins/mix"));
var import_names = __toESM(require("colord/plugins/names"));
var import_lab = __toESM(require("colord/plugins/lab"));
var import_cmyk = __toESM(require("colord/plugins/cmyk"));
var import_hwb = __toESM(require("colord/plugins/hwb"));
(0, import_colord.extend)([import_mix.default, import_lch.default, import_names.default, import_lab.default, import_cmyk.default, import_hwb.default]);
function lchToHsl(n = 0, total = 12, a = 1, s = 20, lightness = 60) {
  let lch = `lch(${lightness}% ${s} ${n * (360 / total)} / ${a})`;
  let hsl = (0, import_colord.colord)(lch).toHslString();
  return hsl;
}
function getColorInfo(color) {
  const cld = (0, import_colord.colord)(color);
  let info = {
    dark: cld.isDark(),
    hex: cld.toHex(),
    rgb: cld.toRgbString(),
...
function chromaColorMix(chroma, tonic, part = 0.3) {
  let hsl = (0, import_colord.colord)(pitchColor(tonic));
  let lch = (0, import_colord.colord)(lchToHsl(tonic, 12, 1));
...
var import_tonal3 = require("@tonaljs/tonal");
var import_core10 = require("@vueuse/core");
var import_core11 = require("@vueuse/core");
var tempo = (0, import_vue.reactive)({
  initialized: false,
  bpm: useClamp((0, import_core10.useStorage)("tempo-bpm", 100), 10, 500),
  blink: false,
  started: false,
  playing: false,
  stopped: false,
  mute: (0, import_core10.useStorage)("tempo-mute", true),
  volume: useClamp((0, import_core10.useStorage)("tempo-volume", 0.5), 0, 1),
  progress: 0,
  position: 0,
  ticks: 0,
  metre: {
    over: 4,
    under: 4,
    num: (0, import_vue.computed)(() => (tempo.metre.over / (tempo.metre.under / 4)).toFixed(2))
  },
  hz: (0, import_vue.computed)(() => (tempo.bpm / 60).toFixed(2)),
  note: (0, import_vue.computed)(() => import_tonal3.Note.pitchClass((0, import_tone5.Frequency)(tempo.hz).toNote())),
  tune: (0, import_vue.computed)(() => {
    return import_tonal3.Note.pitchClass(tempo.note) + 4;
  }),
  pitch: (0, import_vue.computed)(() => freqPitch(tempo.hz)),
  digit: (0, import_vue.computed)(() => ((0, import_tone5.Frequency)(tempo.hz).toMidi() + 12 * 10 + 3) % 12),
  color: (0, import_vue.computed)(() => pitchColor(tempo.digit)),
  tap: {
    last: 0,
    diff: 0,
    timeout: 2e3,
...
function useTempo() {
  ...
  (0, import_vue.onMounted)(() => {
    ...
    metro.pluck = new import_tone5.Sampler({
      urls: {
        E1: "/logic/high.wav",
        E2: "/logic/low.wav"
      },
    ...
    metro.loop = new import_tone5.Loop((time) => {
    ...
    (0, import_core11.useRafFn)(() => {
      tempo.position = import_tone5.Transport.position;
      tempo.ticks = import_tone5.Transport.ticks;
      tempo.progress = metro.loop.progress;
    });
  });
  (0, import_vue.watch)(() => tempo.volume, (vol) => metro.pluck.volume.rampTo((0, import_tone5.gainToDb)(tempo.volume)));
  (0, import_vue.watch)(() => tempo.bpm, (bpm) => import_tone5.Transport.bpm.rampTo(bpm, "4n"), { immediate: true });
  (0, import_vue.watch)(() => tempo.stopped, (stop) => {
    if (stop) {
      import_tone5.Transport.stop();
      tempo.playing = false;
    }
  });
  (0, import_vue.watch)(() => tempo.playing, (playing) => {
    if (playing) {
      if (!tempo.started) {
        (0, import_tone5.start)();
        tempo.started = true;
      }
      tempo.stopped = false;
      import_tone5.Transport.start();
    } else {
      import_tone5.Transport.pause();
    }
  }, {
    immediate: true
  });
...
var import_midi_writer_js = require("midi-writer-js");
var import_midi4 = require("@tonejs/midi");
...
// .vitepress/use/loop.js
var import_midi_writer_js2 = require("midi-writer-js");
var loops = reactive([]);
function useLoop(order = 0) {
  ...
  const panner = new import_tone6.PanVol(loop.pan, 0).connect(channel);
  const synth2 = new import_tone6.PolySynth({
    envelope: {
      attack: 0.5,
      release: 0.2
    },
  ...
  let sequence = new import_tone6.Sequence((time, step) => {
    beatClick(step, time);
  }, loop.steps, loop.metre.under + "n").start(0);
  watch(() => loop.metre.under, () => {
    sequence.stop().dispose();
    sequence = new import_tone6.Sequence((time, step) => {
      beatClick(step, time);
    }, loop.steps, loop.metre.under + "n").start(0);
  ...
  watchEffect(() => {
    sequence.probability = loop.probability;
    panner.volume.targetRampTo((0, import_tone6.gainToDb)(loop.volume), 1);
    panner.pan.targetRampTo(loop.pan, 1);
  });
  function beatClick(step, time) {
    if (import_tone6.context.state == "suspended") {
      (0, import_tone6.start)();
    }
    let notes2 = Object.entries(step).map((entry) => {
      if (entry[0] == "sub")
        return;
      return entry[1] ? (0, import_tone6.Midi)(Number(entry[0]) + loop.tonic) : null;
    }).filter(Number);
    synth2.triggerAttackRelease(notes2, { [loop.metre.under + "n"]: 1 / (step.sub || 1) }, time);
    import_tone6.Draw.schedule(() => {
      let dur = (0, import_tone6.Time)({
...
var import_core12 = require("@vueuse/core");
var import_tone7 = require("tone");
var import_core13 = require("@vueuse/core");
var mic = reactive({
  initiated: false,
  open: false,
  opened: false,
  monitor: false,
  meter: 0,
  volume: (0, import_core13.useClamp)(useStorage("mic-vol", 1), 0, 2),
  gate: (0, import_core13.useClamp)(useStorage("mic-gate", -60), -100, -40)
});
var meter;
var input;
var gate;
...
function useMic() {
  if (!mic.initiated) {
    meter = new import_tone7.Meter();
    meter.normalRange = true;
    input = new import_tone7.UserMedia();
    compressor = new import_tone7.Compressor({ threshold: -20, ratio: 2 }).connect(meter);
    gate = new import_tone7.Gate({ threshold: -60, smoothing: 1 });
    input.connect(gate);
    ...
    watch(() => mic.volume, (vol) => input.volume.rampTo((0, import_tone7.gainToDb)(vol)), { immediate: true });
...
var import_core14 = require("@vueuse/core");
function useSvgMouse() {
  const svg = ref(null);
  const area = ref(null);
  const { pressed } = (0, import_core14.useMousePressed)();
  ...
  onMounted(() => {
    ...
    const { isOutside } = (0, import_core14.useMouseInElement)(area);
    watch(isOutside, (out) => {
      mouse.inside = !out;
    });
  });
...
var import_tone8 = require("tone");
var import_core15 = require("@vueuse/core");
...
var options = (0, import_core15.useStorage)("noise-options", {
  noise: {
    type: "pink"
  },
  envelope: {
...
var filterOptions = (0, import_core15.useStorage)("filter-options", {
  on: false,
  play: false,
  volume: 0.5,
  baseFrequency: 50,
...
var pannerOptions = (0, import_core15.useStorage)("panner-options", {
  on: false,
  play: false,
  wet: 1,
  frequency: 1,
...
var crusherOptions = (0, import_core15.useStorage)("bit-options", {
  on: false,
  bits: 16,
  wet: 1,
  volume: 1
...
function useNoise() {
  ...
  const fft = new import_tone8.FFT({ size: 512, smoothing: 0.2 }).connect(channel);
  for (let j = 0; j < 32; j++) {
    fftFreq.value[j] = fft.getFrequencyOfIndex(j);
  }
  const gain = new import_tone8.Gain(options.value.volume).connect(fft);
  const filterGain = new import_tone8.Gain(filterOptions.value.volume).connect(fft);
  const pannerGain = new import_tone8.Gain(pannerOptions.value.volume).connect(fft);
  const crusherGain = new import_tone8.Gain(crusherOptions.value.volume).connect(fft);
  const panner = new import_tone8.AutoPanner(pannerOptions.value).connect(pannerGain);
  const crusher = new import_tone8.BitCrusher(crusherOptions.value).connect(crusherGain).connect(panner);
  const filter = new import_tone8.AutoFilter(filterOptions.value).connect(filterGain).connect(crusher);
  const synth2 = new import_tone8.NoiseSynth(options.value).connect(gain).connect(filter);
  const { pause, resume } = (0, import_core15.useRafFn)(() => {
    let arr = fft.getValue();
    for (let j = 0; j < 32; j++) {
      fftData.value[j] = (0, import_tone8.dbToGain)(arr[j]) * 10;
    }
  });
  (0, import_core15.onKeyStroke)(" ", (e) => {
    e.preventDefault();
    active.value = true;
  }, { eventName: "keydown" });
  (0, import_core15.onKeyStroke)(" ", (e) => {
...
function init2() {
  (0, import_tone9.start)();
  if (piano)
    return;
  const { channel } = createChannel("piano");
  piano = new import_tone9.Sampler({
    urls: {
      A0: "A0.mp3",
      C1: "C1.mp3",
      "D#1": "Ds1.mp3",
...
function pianoAttack(note, velocity) {
  if (!piano)
    return init2();
  piano.triggerAttack(note, (0, import_tone9.now)(), velocity);
}
...
var import_core16 = require("@vueuse/core");
var import_tone10 = require("tone");
var tracks = reactive([]);
function useSequence(metre = {
  ...
  audio2.panner = new import_tone10.PanVol(pan, 0).connect(channel);
  audio2.synth = new import_tone10.Sampler({
    urls: {
      A1: "tongue/high.wav",
      A2: "tongue/low.wav",
      B1: "synth/high.wav",
  ...
  audio2.recorder = new import_tone10.Recorder();
  const recorder2 = reactive({
    ...
    async rec(pos = "main") {
      if (!recorder2.recording) {
        audio2.meter = new import_tone10.Meter().connect(audio2.recorder);
        audio2.mic = new import_tone10.UserMedia(1).connect(audio2.meter);
  ...
  let sequence = new import_tone10.Sequence((time, step) => {
    beatClick(step, time);
  }, steps, metre.under + "n").start(0);
  watch(() => metre.under, () => {
    sequence.stop().dispose();
    sequence = new import_tone10.Sequence((time, step) => {
      beatClick(step, time);
    }, steps, metre.under + "n").start(0);
  });
  watch(() => metre.over, () => {
  ...
  watch(volume, (vol) => {
    audio2.panner.volume.targetRampTo((0, import_tone10.gainToDb)(vol), 1);
  }, { immediate: true });
  watch(panning, (p) => {
  ...
  function beatClick(step, time) {
    if (import_tone10.context.state == "suspended") {
      (0, import_tone10.start)();
    }
    let mainStep = typeof step == "string" ? +step.split("-")[0] : step;
    import_tone10.Draw.schedule(() => {
      current.value = step;
    }, time);
  ...
  const lastHit = ref(0);
  (0, import_core16.onKeyStroke)("Shift", () => {
