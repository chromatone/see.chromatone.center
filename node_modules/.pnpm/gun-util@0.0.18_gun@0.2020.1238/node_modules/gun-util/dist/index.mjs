...
class Auth {
    ...
    join(options = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (timeout) {
                yield timeoutAfter(joins, timeout).catch(err => {
                    stop = true;
                    throw err;
                });
            }
    ...
    _addOnAuthCallback(cb) {
        if (typeof cb !== 'function') {
            throw new Error('Unvalid auth callback function. Pass a function auth.on().');
        }
...
function _crypt(data, map, opts) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!secret && epub) {
            secret = yield Gun$1.SEA.secret(epub, pair || secret);
...
const _encryptValue = (value, { secret, signed }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    let data = yield Gun$1.SEA.encrypt(value, secret);
    if (typeof data === 'undefined') {
        throw _getSEAError('Could not encrypt');
    }
    if (signed) {
        data = yield Gun$1.SEA.sign(data, secret);
        if (typeof data === 'undefined') {
...
const _decryptValue = (data, { secret, signed }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    let msg = data;
    if (signed) {
        msg = yield Gun$1.SEA.verify(data, secret);
        if (typeof msg === 'undefined') {
            throw _getSEAError('Could not verify');
        }
    }
    let value = yield Gun$1.SEA.decrypt(msg, secret);
    if (typeof value === 'undefined') {
...
const _getSEAError = (_default) => {
    let err = Gun$1.SEA.err || _default;
    if (!err) {
        return undefined;
    }
...
export { ALL_DATE_UNITS, Auth, AuthError, DATE_UNIT_SET, DateTree, Gun, GunError, InvalidCredentials, MultipleAuthError, NetworkError, TEST_GUN_OPTIONS, TEST_GUN_PEERS, TimeoutError, UserExists, _crypt, closedFilter, decrypt, delay, encrypt, errorAfter, filterWithRange, filteredIndexRange, fixSea, gunLogOnceFix, isGunInstance, isInRange, isPlatformWeb, isValueRangeEmpty, iterateAll, iterateItems, iterateKeys, iterateRecord, iterateRefs, iterateValues, mapValueRange, rangeWithFilter, subscribe, timeoutAfter, waitForData };
